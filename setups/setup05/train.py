...
def pipeline(iterations, warmup=5000, save_every=1000):
    ...
    rastered_source += DilatePoints(labels, dilations=6)
    rastered_source += Relabel(labels)
    rastered_source += Unlabel(labels, unlabelled)
    # rastered_source += gp.MergeProvider()
    # rastered_source += gp.RandomLocation(mask=labels_mask, min_masked=0.1)
    ...
    # voxel_source += gp.RandomLocation(mask=labels_mask, min_masked=0.1)

    mixed_source = rastered_source + voxel_source
    mixed_source += gp.RandomProvider(probabilities=[0.8, 0.2])
    ...
    training_pipeline += gp.Squeeze([raw])
...
if __name__ == "__main__":
    # pipeline(50000, 5000, 5000)
    pipeline(50000, 0, 5000)
