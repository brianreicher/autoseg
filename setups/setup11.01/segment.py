...
def segment(
    ...
    # frag_array = seeded_mutex_watershed(seeds, affs, offsets)
    # logger.info("Correcting with skeleton...")
    # segmentation = np.zeros_like(frag_array)
    # for frag_id in tqdm(np.unique(frag_array)):
    #     seg_ids = list(np.unique(seeds[frag_array == frag_id]))
    #     if len(seg_ids) == 1:
    #         segmentation[frag_array == frag_id] = seg_ids[0]
